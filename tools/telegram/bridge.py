...
A Telegram bot that pipes multimodal input (text, images, voice, files) directly
into your local OpenCode session via Minimax.
"""
import os
import re
import asyncio
...
# Handle imports for both old and new paths
...
try:
    ...
    try:
        from tools.lobster.lobster_stream import lobster_stream
    except ImportError:
        lobster_stream = None
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Determine path to web/agent.py - support both old and new paths
    script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(script_dir)
    web_agent_path = os.path.join(parent_dir, "web", "agent.py")
    # Also check legacy path
    if not os.path.exists(web_agent_path):
        web_agent_path = os.path.join(parent_dir, "web_agent.py")
